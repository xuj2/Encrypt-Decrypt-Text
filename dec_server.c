...
void *decrypt(void *args)
{
  ...
  for (int i = 0; i < (strlen(text) - 1); i++)
  {
    ...
    dec_text[i] = (dec_char + 65);
  }
  memset((char *)args, '\0', MAX_CHAR);
  strcpy((char *)args, dec_text);
}
...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    {
      if (data[0] == 101) //reject enc_client
      {
      ...
      else
      {
        if (data[strlen(data) - 1] == 10) //all data received
        {
          while (1) //loop until there's opening for new thread
          {
            ...
            if (pthread_create(&tid[t], NULL, decrypt, (void *)data) != 0)
            {
              t++;
